...
const defaultProducts = [
  {id:'yamaha-psr-e283',sku:'TP-051',name:'Yamaha PSR-E283',category:'Teclados',competitorPrice:1499,price:1049,stock:5,image:'https://images.unsplash.com/photo-1511379938547-c1f69419868d?q=80&w=1200&auto=format&fit=crop',description:'Teclado arranjador Yamaha com 61 teclas, timbres realistas, ritmos automáticos e ótimo custo-benefício para estudo, igreja e apresentações.',tag:'30% OFF NO PIX',rating:'4.9',featured:true},
  {id:'yamaha-p45',sku:'TP-054',name:'Yamaha P-45',category:'Pianos Digitais',competitorPrice:4999,price:3499,stock:3,image:'https://images.unsplash.com/photo-1520523839897-bd0b52f945a0?q=80&w=1200&auto=format&fit=crop',description:'Piano digital Yamaha com 88 teclas pesadas, timbres expressivos e design compacto para estudo avançado, igrejas e apresentações profissionais.',tag:'DESTAQUE',rating:'5.0',featured:true},
  {id:'roland-fp10',sku:'TP-059',name:'Roland FP-10',category:'Pianos Digitais',competitorPrice:5299,price:3709,stock:4,image:'https://images.unsplash.com/photo-1510915361894-db8b60106cb1?q=80&w=1200&auto=format&fit=crop',description:'Piano digital Roland compacto com excelente resposta de teclas, som encorpado e construção premium.',tag:'MAIS VENDIDO',rating:'4.9',featured:true},
  {id:'akai-mpk-mini',sku:'TP-060',name:'Akai MPK Mini MK3',category:'MIDI',competitorPrice:699,price:489,stock:8,image:'https://images.unsplash.com/photo-1511379938547-c1f69419868d?q=80&w=1200&auto=format&fit=crop',description:'Controlador MIDI compacto para produção musical, beats, home studio e criação profissional.',tag:'OFERTA PIX',rating:'4.8',featured:true}
];
...
function App(){
  ...
  const filtered=useMemo(()=>products.filter(p=>{
    ...
    const searchOk = p.name.toLowerCase().includes(search.toLowerCase()) || p.sku.toLowerCase().includes(search.toLowerCase());
    ...
    <section style={{position:'relative',minHeight:540,backgroundImage:'linear-gradient(90deg,rgba(0,0,0,.98) 0%,rgba(0,0,0,.86) 38%,rgba(0,0,0,.2) 100%), url(https://images.unsplash.com/photo-1520523839897-bd0b52f945a0?q=80&w=1600&auto=format&fit=crop)',backgroundSize:'cover',backgroundPosition:'center right',borderBottom:'1px solid rgba(212,162,76,.35)'}}>
      <div style={{maxWidth:1280,margin:'auto',padding:'60px 22px'}}>
        <div style={{maxWidth:670}}>
          <div style={{display:'flex',alignItems:'center',gap:10,color:gold,marginBottom:12}}><div style={{height:1,background:gold,width:120}}></div><span>♛</span><div style={{height:1,background:gold,width:120}}></div></div>
          ...
          <div style={{height:1,background:gold,width:420,margin:'18px 0'}}></div>
    ...
    <main style={{maxWidth:1280,margin:'auto',padding:'0 22px 60px'}}>
      <section style={{border:'1px solid rgba(212,162,76,.5)',borderRadius:12,marginTop:0,padding:'20px 28px 30px',background:'rgba(0,0,0,.75)'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:22}}>
          <div><p style={{margin:0,color:gold,fontWeight:900,textTransform:'uppercase',letterSpacing:1}}>Vitrine Premium</p><h3 style={{fontFamily:'Georgia, serif',fontSize:31,margin:0}}>Melhores ofertas em destaque</h3></div>
          <button onClick={()=>setCategory('Destaques')} style={{background:'transparent',border:`1px solid ${gold}`,color:gold,borderRadius:6,padding:'13px 25px',fontWeight:900}}>VER DESTAQUES</button>
        </div>
        <ProductGrid products={featured.length?featured:filtered.slice(0,8)} makeOrder={makeOrder}/>
      </section>

      <section style={{marginTop:35}}>
        <h3 style={{fontFamily:'Georgia, serif',fontSize:30}}>Catálogo completo</h3>
        <ProductGrid products={filtered} makeOrder={makeOrder}/>
      </section>
...
function ProductGrid({products,makeOrder}){
  return <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:22}}>
    {products.map(p=><article key={p.id} style={{background:'linear-gradient(180deg,#0c0c0c,#050505)',border:'1px solid rgba(212,162,76,.45)',borderRadius:16,overflow:'hidden',boxShadow:'0 18px 45px rgba(0,0,0,.55)'}}>
      <div style={{height:210,position:'relative',background:'#030303'}}><img alt={p.name} src={p.image} style={{width:'100%',height:'100%',objectFit:'cover',opacity:.92}}/><span style={{position:'absolute',top:12,left:12,background:'linear-gradient(135deg,#9a670e,#e0b45b)',color:'#fff',borderRadius:4,padding:'7px 11px',fontWeight:900,fontSize:12}}>{p.tag||'30% OFF NO PIX'}</span></div>
      <div style={{padding:18}}>
        ...
        <p style={{fontSize:13,color:'#aaa'}}>Estoque: {p.stock} | desconto Pix</p>
...
function Admin({products,setProducts,orders,setOrders,goHome}){
  ...
  function submit(e){e.preventDefault();const item={...form,id:form.id||slug(form.name)||'produto-'+Date.now(),competitorPrice:Number(form.competitorPrice||0),price:Number(form.price||price30(form.competitorPrice)),stock:Number(form.stock||0),featured:!!form.featured};const exists=products.some(p=>p.id===item.id);setProducts(exists?products.map(p=>p.id===item.id?item:p):[item,...products]);setForm(empty);alert('Produto salvo!')}
  function importProducts(){try{const list=JSON.parse(importText);if(!Array.isArray(list)){alert('O JSON precisa ser uma lista.');return}const normalized=list.map((p,i)=>({id:p.id||slug(p.name||p.produto||('produto-'+Date.now()+'-'+i)),name:p.name||p.produto||'',sku:p.sku||('TP-'+Date.now()+i),category:p.category||p.categoria||'Teclados',competitorPrice:Number(p.competitorPrice||p.precoConcorrente||p.concorrente||0),price:Number(p.price||p.preco||p.precoTeclaPrime||price30(p.competitorPrice||p.precoConcorrente||0)),stock:Number(p.stock||p.estoque||1),image:p.image||p.imagem||'',description:p.description||p.descricao||'',tag:p.tag||'30% OFF NO PIX',rating:p.rating||'4.8',featured:!!p.featured})).filter(p=>p.name);setProducts([...normalized,...products]);setImportText('');alert(normalized.length+' produtos importados!')}catch(e){alert('JSON inválido. Copie tudo novamente.')}}

  if(!logged)return <div style={{minHeight:'100vh',display:'grid',placeItems:'center',background:'#050505',fontFamily:'Arial'}}><div style={{background:'#0b0b0b',border:`1px solid ${gold}`,color:'#fff',borderRadius:20,padding:32,maxWidth:380,width:'92%'}}><h1>Painel TeclaPrime</h1><p>Digite a senha.</p><input type='password' value={password} onChange={e=>setPassword(e.target.value)} placeholder='Senha' style={{width:'100%',padding:14,border:`1px solid ${gold}`,borderRadius:10,background:'#050505',color:'#fff'}}/><button onClick={login} style={{width:'100%',border:0,background:gold,color:'#fff',borderRadius:10,padding:14,fontWeight:900,marginTop:10}}>Entrar</button><small>Senha inicial: 123456</small></div></div>;
